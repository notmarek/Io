export const dbgr = async (path, self, renderModule, log) => {
	...
	after_hook(window.session, "set", (k, v) => { log("session.set", `Set '${k}' to '${v}'`);
												  document.getElementById("dbgr-cache-size").innerHTML = Object.keys(window.session).length - 4;
											      document.getElementById("dbgr-cache-inv").innerHTML = window.session.invalid.length;
												});
	after_hook(window.session, "invalidate", (k) => { log("session.invalidate", `Invalidated ${k}`);
													  document.getElementById("dbgr-cache-inv").innerHTML = window.session.invalid.length;
													});
  	after_hook(window.session, "get", (k, res) => { log("session.get", `Cache ${res ? "hit" : "miss"} on key "${k}".`); 
	...
	before_hook(localStorage, "setItem", (k, v) => log("localStorage.setItem", `Set '${k}' to '${v}'`), true);
...
const before_hook = (context, fn_name, before, proto = false) => {
	...
	replace[fn_name] = (...args) => { before.apply(context, args); return fn_og.apply(context, args); };
}
